...
public class LevelController {
    ...
    private void setListeners() {
        ...
        scene.setOnKeyPressed(event -> {
            if ((event.getCode() == KeyCode.UP) || event.getCode() == KeyCode.W) {
                ...
                levelData.getBomberman().setImageView(Constants.CHARACTER_U);

            }

            if ((event.getCode() == KeyCode.DOWN) || event.getCode() == KeyCode.S) {
                levelData.getBomberman().moveDown();
                levelData.getBomberman().setImageView(Constants.CHARACTER_IMAGE);
            }

            if ((event.getCode() == KeyCode.LEFT) || event.getCode() == KeyCode.A) {
                levelData.getBomberman().moveLeft();
                levelData.getBomberman().setImageView(Constants.CHARACTER_L);
            }

            if ((event.getCode() == KeyCode.RIGHT) || event.getCode() == KeyCode.D) {
                levelData.getBomberman().moveRight();
                levelData.getBomberman().setImageView(Constants.CHARACTER_R);
            }
    ...
    //проходит по массиву и если должно быть изменено
    ...
    private void endOfTheGame(final String imagePath) {
